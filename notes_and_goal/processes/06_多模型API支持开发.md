...
**降级策略**：如果 `GET /models` 请求失败，返回空列表，用户需手动输入模型名称。UI 显示红色提示"获取失败，请检查 API URL 和 API Key，或手动输入模型名称"。
...
- 定义 `LLMProvider` 数据类（name, apiUrl, defaultModel, description）
- 预置所有支持的提供商配置
- 移除 `supportsJsonMode` 字段（不再需要，统一用 prompt 强制 JSON）
- 移除 `models` 预设列表（改为动态获取，失败返回空列表）
...
- 修改 `AIService`：
  ...
  - 新增 `fetchModels()` 方法：从厂商 API 动态获取模型列表，失败返回空列表
...
- **模型选择改为动态获取**：
  ...
  - 获取失败时显示红色提示"获取失败，请检查 API URL 和 API Key，或手动输入模型名称"
  - 保留手动输入模型名称的能力（始终可用）
- 保留自定义选项，支持手动输入所有参数
...
- [x] fetchModels() 正确推导 models 端点 URL
- [x] fetchModels() 正确解析返回的模型列表
- [x] fetchModels() 请求失败时返回空列表
- [x] UI 正确展示动态获取的模型列表
- [x] 获取失败时显示红色提示文字
- [x] 手动输入模型名称仍然可用
